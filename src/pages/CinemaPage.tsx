            ...
            <Grid item xs={12} sm={6} md={4} key={cinema.id}>
              <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column',
                ...
                <CardContent sx={{ flexGrow: 1 }}>
                  <Typography variant="h6" sx={{ fontWeight: 700, mb: 1 }}>{cinema.name}</Typography>
                  <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 0.5 }}>
                    <LocationOnIcon sx={{ color: 'primary.main', fontSize: 18, mt: 0.2 }} />
                    <Typography variant="body2" color="text.secondary">{cinema.location}</Typography>
                  </Box>
                </CardContent>
                <CardActions sx={{ p: 2, pt: 0 }}>
                  <Button variant="outlined" fullWidth size="small" href={`/schedule?cinema_id=${cinema.id}`}>View Schedules</Button>
                </CardActions>
              </Card>
            </Grid>
          ))}
